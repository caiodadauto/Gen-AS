...
from dggi.generator.training import train as _train
from dggi.generator.evaluation import generate as _generate
from dggi.generator.evaluation import evaluate as _evaluate
...
@click.option(
    ...
    type=str,
    default=None,
    help="Path to the directory with configuraion file. If not provided, the current directory will be considered if a file `CONFIG_NAME.yaml` exists, otherwise, `$HOME/.config/dggi_dggm/` will be considered.",
)
@click.option(
    "-cn",
    "--config-name",
...
@click.pass_context
def train(ctx):
    """Train the model based on a dggi configuration."""
...
def generate(ctx, min_number_nodes, max_number_nodes, number_graphs):
    """Generate synthetic graphs based on the dggi configurations."""
    initialize_config_dir(
        version_base=None, config_dir=ctx.obj["config_dir"], job_name="train"
    )
    cfg = compose(config_name=ctx.obj["config_name"])
