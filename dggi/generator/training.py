...
from dggi.generator.loading import create, split_data, GraphSequenceSampler
from dggi.generator.evaluation import bootstrap_eval
from dggi.generator.model import PlainGRU
...
from dggi.generator.train_utils import (
    checkpoint,
    save_best,
...
def epoch_training(
    ...
    bar.set_suffix(f"0 / {n_batches} batches")
    for batch_idx, data in enumerate(data_loader):
        ...
        bar.set_suffix(f"{batch_idx} / {n_batches} batches")
        bar.update()
        if progress_bar_qt is not None and progress_bar_qt.stop_running:
            break
    bar.close()
